...
class TestAsmasterListener(object):
    ...
    def test_long_system_messages(self):
        message = 'a' * 1000
        stream = self.create_stream('{"message": "' + message + '"}')
        self.handler.on_complete_message({'type': 'system'}, stream)
        assert len(self.new_lines('system')) == 1

    def test_download_file_database_info(self):
        stream = self.create_stream(SAMPLE_FILE_CONTENTS)
        self.handler.on_complete_message(DOWNLOAD_FILE_HEADER, stream)
    ...
    def test_feeds(self):
        ...
        self.handler.on_complete_message(header, feed)
